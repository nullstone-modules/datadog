...
resource "aws_cloudwatch_log_subscription_filter" "app" {
  ...
  lifecycle {
    precondition {
      condition     = local.app_type != "unknown"
      error_message = "Could not tell whether this app is ECS/Fargate or Lambda: its app_metadata has neither `task_definition_name` nor `function_name`. Set the `app_type` variable to `ecs` or `lambda` explicitly."
    }
  }
}
