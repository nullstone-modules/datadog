# Datadog OpenTelemetry (Direct) — AWS
...
connected **datadog datastore** (`aws-datadog`); the API key is read from Secrets Manager at apply
time and injected as a secret env var.

For the sidecar alternative on ECS, see `aws-ecs-otel-datadog-agent`. For cluster-wide delivery on
EKS, see `aws-k8s-otel-datadog-extender`. Don't attach more than one of these to the same app — they
all configure the SDK's exporter.
...
> The ECS path (`aws-ecs-datadog` + `aws-ecs-otel-datadog-agent`) does not need it.
...
| `keep_console_logs` | `true` | Keep logs on stdout as well, so CloudWatch keeps working. |
